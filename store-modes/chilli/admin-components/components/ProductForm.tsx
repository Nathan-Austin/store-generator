...
const HEAT_LEVELS = ['Mild', 'Medium', 'Hot', 'Extreme'];
...
export default function ProductForm({
  ...
  const selectedChilliTypes = product?.chilliTypes?.map((ct) => String(ct.id)) ?? [];
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <label className="block text-sm font-medium text-text-secondary">{t('form.heatLevel')}</label>
              <select
                name="heat_level"
                defaultValue={product?.heatLevel ?? ''}
                className="w-full rounded-xl border border-border bg-white px-4 py-2"
              >
                <option value="">{t('form.selectHeat')}</option>
          ...
          <div className="space-y-2">
            <label className="block text-sm font-medium text-text-secondary">{t('form.chilliTypes')}</label>
            <select
              name="chilliTypeIds"
              multiple
              defaultValue={selectedChilliTypes}
              className="w-full rounded-xl border border-border bg-white px-4 py-2 h-32"
            >
              {chilliTypes.map((chilli) => (
                <option key={chilli.id} value={chilli.id}>
                  {chilli.name}
                </option>
              ))}
            </select>
            <p className="text-xs text-text-muted">{t('form.multiSelectHint')}</p>
          </div>
